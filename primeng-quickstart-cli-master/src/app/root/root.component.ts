...
export class RootComponent implements OnInit {
  ...
  private readonly storeNames: { [key: string]: string } = {
    ...
    '25': 'Epic Games', '29': 'IndieGala', '30': 'Blizzard'
  };

  constructor(
    private chiamateApi: ChiamateAPIService,
    private gameSearch: GameSearchService,
  ...
  private loadHomeDeals(): void {
    ...
    this.chiamateApi.getDeals().subscribe({
      next: (results: CheapSharkDeal[]) => {
        ...
        this.loading = false;
        if (this.games.length === 0) this.errorMessage = 'Non sono riuscito a trovare giochi popolari attualmente in offerta.';
      },
      error: (error) => {
  ...
  search(): void {
    ...
    this.gameSearch.search(searchQuery).subscribe({
      next: (results: CheapSharkDeal[]) => {
        console.log('Risultati ricerca giochi:', results);
        this.games = this.buildUniqueGames(results);
        this.loading = false;
        if (this.games.length === 0) this.errorMessage = `Nessun gioco trovato per "${searchQuery}"`;
      },
      error: (error) => {
  ...
  private buildUniqueGames(results: CheapSharkDeal[]): RootGame[] {
    ...
    for (const deal of results) {
      if (!deal.gameID) continue;
      const existing = uniqueGames.get(deal.gameID);
      const currentPrice = Number.parseFloat(deal.salePrice);
      const existingPrice = existing ? Number.parseFloat(existing.salePrice) : Number.POSITIVE_INFINITY;
      if (!existing || currentPrice < existingPrice) uniqueGames.set(deal.gameID, deal);
    }
  ...
  toggleWishlist(gameID: string): void {
    if (this.wishlist.has(gameID)) this.wishlist.delete(gameID);
    else this.wishlist.add(gameID);
  }
